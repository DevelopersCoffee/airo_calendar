...
public final class PlatformCalendarPlugin: NSObject, FlutterPlugin, EKEventEditViewDelegate {
  ...
  private func listCalendars(result: FlutterResult) {
    ...
    result(["calendars": calendars, "adapter": "ios"])
  }
  ...
  private func getEvents(arguments: [String: Any]?, result: FlutterResult) {
    ...
    let payload = Array(events.prefix(maxResults)).map(serialize(event:))
    result([
      "events": payload,
      "adapter": "ios",
  ...
  private func createCalendarEvent(arguments: [String: Any]?, result: @escaping FlutterResult) {
    ...
    if arguments["start"] != nil && arguments["end"] != nil {
      guard let title = arguments["title"] as? String, !title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
        ...
        result([
          "error": "invalid_calendar_event",
          "message": "Calendar event requires title, start, and end.",
        ])
        return
      }
      DispatchQueue.main.async {
        ...
        self.pendingCreateResult = result
  ...
  public func eventEditViewController(
    ...
    let result = pendingCreateResult
